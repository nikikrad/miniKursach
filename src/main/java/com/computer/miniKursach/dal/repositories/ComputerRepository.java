...
public class ComputerRepository implements IComputerRepository {
    public void create(ComputerEntity entity) throws SQLException {
        DBConnection.statement.executeUpdate(String.format("INSERT INTO computer (ram, hdd, ssd, matherboard, power_unit, system_unit, video_card, price) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') ", entity.ram, entity.hdd,entity.ssd,entity.matherboard,entity.power_unit,entity.power_unit,entity.video_card, entity.price));
    }
    public void update(ComputerEntity entity) throws SQLException {
        DBConnection.statement.executeUpdate(String.format("UPDATE computer set ram = '%s', hdd = '%s',ssd = '%s',matherboard = '%s',power_unit = '%s',system_unit = '%s',video_card = '%s', price = '%s' where id = '%s'", entity.ram, entity.hdd,entity.ssd,entity.matherboard,entity.power_unit,entity.power_unit,entity.video_card, entity.price, entity.id));
    }
    ...
    public ArrayList<ComputerEntity> getAll() throws SQLException {
        ...
        while (resultSet.next())
        {
            ...
            computer.price = resultSet.getDouble("price");
            result.add(computer);
        }
        return result;
    }
